Companies that often hire English + Spanish speakers in Poland
In Poznań, Warsaw, Kraków and Wrocław:
Teleperformance
Concentrix
Majorel (now part of TP Infinity / Teleperformance)
Accenture Operations
Genpact
Capgemini
Cognizant
Foundever
Alorica
Keywords Studios (Trust & Safety / Content Moderation)
Amazon customer support operations
TikTok vendors and moderation partners

The highest-paying roles generally aren't standard customer service. They tend to be:
Trust & Safety
Content Moderation
Quality Analyst
Escalations
Risk/Fraud Operations
Workforce Management
Training
Team Leader


After a year of TikTok onboarding experience, your market value is likely much higher than today because you'll have:
TikTok ecosystem knowledge
CRM experience
customer onboarding experience
KPI experience
multilingual support experience


CV Update
Since you only started on 2 September 2026, keep it honest and focused on responsibilities rather than achievements you haven't had time to demonstrate yet.

CV Entry (Current Role)

TikTok Leads Onboarding Specialist
 Teleperformance Poland (for ByteDance/TikTok)
 September 2026 – Present

Support onboarding and activation of new TikTok Leads clients and advertisers.
Assist customers with account setup, platform navigation, and onboarding processes.
Handle customer inquiries via internal support channels while adhering to service-level agreements (SLAs).
Research and apply standard operating procedures (SOPs) to resolve customer issues and provide accurate guidance.
Escalate complex cases to appropriate teams following established workflows.
Maintain accurate case documentation and customer records in internal systems.
Support a multilingual customer base in English and Spanish.
Alternative (More Professional)

Customer Support Specialist | TikTok Leads Onboarding
 Teleperformance Poland (Client: ByteDance/TikTok)
 September 2026 – Present

Provide onboarding support for businesses using TikTok Leads solutions.
Guide customers through account setup and product adoption.
Investigate customer issues using knowledge bases, SOPs, and internal tools.
Ensure compliance with quality standards and operational procedures.
Collaborate with cross-functional teams to resolve escalated cases.
Deliver support in English and Spanish.
Skills to Add

After a few months, you can add:

Customer Onboarding
Account Management Support
CRM Systems
Case Management
Customer Experience (CX)
Process Compliance
SOP Navigation
Escalation Management
English (Native)
Spanish (Native)
Remote Customer Support
What not to write yet

Avoid claiming things like:

KPI achievement
Training new agents
Process improvement
Subject Matter Expert (SME)
Quality Analyst responsibilities

until you've actually done them.

For someone returning to work after a 5-year parental break, I'd position this role as:
Multilingual Customer Support & Onboarding Specialist supporting TikTok business customers in English and Spanish.
That wording will look stronger when you start applying for higher-paying roles in 6-12 months.

After receiving a formal offer:
Thank you very much for the offer. I am genuinely interested in joining the team. While 7,000 PLN gross was my initial expectation based on the information available when I applied, I now have a fuller understanding of the responsibilities, working arrangements, and scope of the position. Considering my native-level English and Spanish, current TikTok onboarding experience, and relevant HubSpot and Zendesk credentials, would there be any flexibility to increase the base salary to 7,800 PLN gross?
I would also appreciate clarification regarding the standard working hours and whether there is flexibility in start and finish times after the training period. I have regular childcare responsibilities on certain afternoons, so understanding the team’s scheduling arrangements is important to me.

Thank you very much for the offer. I am genuinely interested in joining the team. While 7,000 PLN gross was my initial expectation based on the information available when I applied, I now have a fuller understanding of the responsibilities, working arrangements, and scope of the position.
Considering my native-level English and Spanish, current TikTok onboarding experience, and relevant HubSpot and Zendesk credentials, would there be any flexibility to increase the base salary to 7,800 PLN gross?

Could you please explain the team’s standard working hours and whether employees have any flexibility in their start and finish times after training? I have regular childcare responsibilities on certain afternoons, so it would be helpful to understand whether, for example, starting and finishing earlier on those days could be accommodated.

During the second interview, ask:
I understand that this position is offered on a 14-month fixed-term contract. HR mentioned that the company may be considering permanent opportunities for Spanish-speaking employees. Could you tell me whether there may be a pathway to a permanent position after the initial contract and what would normally determine that decision?
